...
def monai_layout():
    ...
    monai_training, monai_inference = st.tabs(["__Training__", "__Inference__"])

    with monai_training:
        training_sidebar, training_main, training_rsb = st.columns([1, 6, 1.5])
    ...
    with monai_inference:
        inference_sidebar, inference_main, inference_rsb = st.columns([1, 6, 1.5])
